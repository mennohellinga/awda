...
def describetable_LT(connection, name):
    ...
    f = open(LT_path+name+'_description.tex', 'w+')
    f.write(r'''
            \section{'''+sanitise_LT(name)+r''' --- Table Description}
            Column list generated by describetable\_LT(\emph{connection\_name}, "'''+sanitise_LT(name)+r'''").
    ...
    for t in c.execute('''pragma table_info(%s)''' % name):
        f.write(repr(t[0])+'&'+sanitise_LT(t[1])+'&'+sanitise_LT(t[2])+'\\\\\n')
